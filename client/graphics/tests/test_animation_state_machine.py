...
No OpenCV, no sprite files, no logic imports — the loader is stubbed.
"""
import sys, os
...
def _loader():
    ...
    def _get(key, folder):
        anim = MagicMock()
        anim.frame_at.return_value = MagicMock()
        return anim
...
def test_initial_folder_is_none():
    assert AnimationState("wR", _loader())._current_folder is None


def test_initial_animation_is_none():
    assert AnimationState("wR", _loader())._current_animation is None


def test_sprite_key_stored():
    assert AnimationState("wR", _loader())._piece_key == "wR"
