...
class Layout:
    ...
    def _recompute(self):
        available_w = max(1, self.window_px_w - gfx_config.SIDEBAR_PX_W)
        available_h = max(1, self.window_px_h)
        ...
        self.board_origin_x = (available_w - self.board_px_w) // 2
        self.board_origin_y = (available_h - self.board_px_h) // 2
        self.sidebar_origin_x = available_w

    def screen_to_board_pixel(self, screen_x, screen_y):
        bx, by = screen_x - self.board_origin_x, screen_y - self.board_origin_y
        if bx < 0 or by < 0 or bx >= self.board_px_w or by >= self.board_px_h:
            return None   # click landed outside the board (sidebar/margin)
